...
namespace Faraboom.Framework.UI.FormBuilder
{
    ...
    {
        public static string BestPropertyName<THelper>(THelper html, PropertyViewModel vm) where THelper : IFormBuilderHtmlHelper
        {
            var viewname = BestViewName(html.ViewFinder, vm.Type, "FormBuilder/Property.");
           return viewname;
        }
        public static string BestPartialName<THelper>(THelper helper, object model, Type type = null, string prefix = null)where THelper : IFormBuilderHtmlHelper
        {
            if (type == null) type = model.GetType();
            return BestViewName(helper.ViewFinder, type, prefix);
        }
        public static string BestViewName<THelper>(THelper helper, Type type, string prefix = null)where THelper : IFormBuilderHtmlHelper
        {
            return BestViewName(helper.ViewFinder, type, prefix);
        }
        ...
        public static string BestViewName(IViewFinder cc, Type type, string prefix, Func<Type, string> getNameIn)
        {
            ...
            //viewName = viewName ?? "FormBuilder/Property.System.Object"; //must be some unknown object exposed as an interface
