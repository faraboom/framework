...
namespace Faraboom.Framework.Service
{
    public abstract class ServiceBase<T> where T : class
    {
        ...
        protected ServiceBase(IUnitOfWorkProvider unitOfWorkProvider, IHttpContextAccessor httpContextAccessor, ILogger<T> logger)
        {
        ...
        protected IQueryable<TSource> FilterList<TSource>(PagingDto pagingDto, IQueryable<TSource> lst, IEnumerable<(string Column, bool Descending)> orderBy)
        {
            try
            {
                ...
                {
                    lst = lst.OrderBy($"{pagingDto.SortFilter.Column} {(pagingDto.SortFilter.Descending ? "descending" : "")}");
                }
                else if (orderBy is not null && orderBy.Any())
                {
                    lst = lst.OrderBy(string.Join(",", orderBy.Select(t => $"{t.Column} {(t.Descending ? "descending" : "")}")));
                }
        ...
        protected IEnumerable<TSource> FilterList<TSource>(PagingDto pagingDto, IEnumerable<TSource> lst, IEnumerable<(string Column, bool Descending)> orderBy)
        {
            try
            {
                ...
                if (pagingDto.SortFilter is not null)
                {
                    lst = lst.AsQueryable().OrderBy($"{pagingDto.SortFilter.Column} {(pagingDto.SortFilter.Descending ? "descending" : "")}");
                }
                else if (orderBy is not null && orderBy.Any())
                {
                    lst = lst.AsQueryable().OrderBy(string.Join(",", orderBy.Select(t => $"{t.Column} {(t.Descending ? "descending" : "")}")));
                }
